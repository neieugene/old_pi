...
(function() {
    ...
    var changes_aplying = {
        text_align: function() {
            var value = $('[name="content_format[text_align]"]').val();
            ge('.article-preview__title')('textAlign', value);
            ge('.article-preview__info')('textAlign', value);

            if (value == 'center') {
                ge('.article-preview__title')('padding', '0 30px');
            } else if (value == 'left') {
                ge('.article-preview__title')('padding', null);
            } else {
                ge('.article-preview:not(.article-preview--fluid) .article-preview__title')('padding', null);
                ge('.article-preview.article-preview--fluid .article-preview__title')('padding', '0 30px');
            }
        },
        title_color: function() {
            var color = $('[name="content_format[title_color]"]').val();
            $article.attr('data-format_title_color', color);
            ge('.article-preview__title span')('color', color);
            ge('.article-preview__title')('color', color);
        },
        title_background_color: function() {
            var bg_color = $('[name="content_format[title_background_color]"]').val()
            if (bg_color) {
                $article.attr('data-format_title_background_color', bg_color);
                ge('.article-preview__title span')('backgroundColor', bg_color);
                ge('.article-preview__title span')('boxShadow', '-10px 0 0 '+bg_color+', 10px 0 0 '+bg_color);
            } else {
                $article.attr('data-format_title_color', null);
                $article.attr('data-format_title_background_color', null);
                ge('.article-preview__title span')('backgroundColor', null);
                ge('.article-preview__title span')('boxShadow', null);
            }
        },
        author_color: function() {
            ge('.article-preview__info__devider, .article-preview__author')('color', $('[name="content_format[author_color]"]').val());
        },
        category_color: function() {
            ge('.article-preview__category')('color', $('[name="content_format[category_color]"]').val() || format_category_color);
        },
        background_color: function() {
            ge('.article-preview:not(.article-preview--fluid) .article-preview__bg')('backgroundColor', $('[name="content_format[background_color]"]').val());
        },
        background_shadow: function() {
            ge('.article-preview__shadow')('display', $('[name="content_format[background_shadow]"]').is(':checked') ? 'block' : 'none');
        },
        remove_background_shadow: function() {
            var remove = $('[name="content_format[remove_background_image]"]').is(':checked')
            ge('.article-preview:not(.article-preview--fluid) .article-preview__bg')('display', remove ? 'none' : 'block');
        }
    };
    ...
    var loaded = function() {
        $preview = $('#preview');
        $article = $preview.find('.article-preview');
    };

    var ge = function(selector) {
        var elems = $preview.find(selector);
    ...
    var changed_form = function() {
        for (var key in changes_aplying) {
            changes_aplying[key]();
        }
    };

    var changed_image = function() {
        var fr = new FileReader();
        fr.onload = function(e) {
            var b64 = fr.result;
            ge('.article-preview:not(.article-preview--fluid) .article-preview__bg')('backgroundImage', 'url('+b64+')');
            format_img_url = b64;
        };
    ...
    $.document
        .on('change', '[name="content_format[background_image]"]', changed_image)
        .on('change', '.js-formats__form', changed_form)
        .on('keyup', '.js-formats__form', changed_form)
        .ready(loaded)
...
(function() {
    ...
    $(function() {
        var colors = ['#fff', '#1e28c8', '#ffff00', '#5cffea', '#ff6e6e', '#f5f5f5', '#c6c6c6', '#000', '#fee078', '#ff0078', '#ff5050', '#c864f0', '#37ffa0', '#0fbee6', '#ff0000', '#0064ff'];
        var colors_html = '';
        var $color = $('.js-formats__color');
        var $color_content = $color.find('.js-formats__color__content');
        var $input;

        colors.forEach(function(color) {
            colors_html += '<b style="user-select: none;background-color: '+color+';width: 30px; height: 30px; display: inline-block;vertical-align:middle;" data-color="'+color+'"></b>'
        });
        colors_html += '<b data-color="" style="line-height: 30px;vertical-align:middle;display: inline-block;">Удалить</b>'

        var clicked = function() {
            var item = this.dataset.color;

            $input.val(item).focus().closest('form').trigger('change');
        };

        var mouseenter = function() {
            $input = $color.find('input:focus');
        };
        ...
        var focused = function() {
            ...
            $color_content.removeClass('-hold');
        };

        var unfocused = function() {
            timer_focus = setTimeout(function() {
                $color_content.addClass('-hold');
            }, 50);
        };

        $color_content.html(colors_html)
            .on('click', 'b', clicked)
            .on('mouseenter', 'b', mouseenter)

        $color
            .on('focus', 'input', focused)
            .on('blur', 'input', unfocused)
    });
...
(function() {
    $(function() {
        var dateInput = $('.publication_time_field');
        dateInput.blur(function() {
            this.value = roundCurrentTime(this.value);
        });

        function roundCurrentTime(curTime) {
            var data = new Date(curTime),
                min = data.getMinutes();
            if (min !== 0 && min !== 30) {
                data.setMinutes(30 * Math.round(min/30));
            }

            return data.toISOString().split(':').slice(0, 2).join(":");
        }

        var inputs = $('[name="material[start_time]"]');
        if (inputs.length && inputs[2].value) {
            inputs[1].value = new Date(inputs[2].value).toLocaleTimeString('ru', {timeZone: "Europe/Minsk"}).split(":").slice(0, 2).join(":");
        }

        $("#edit_material,#new_material").submit(function() {
            var inputs = $('[name="material[start_time]"]');
            inputs[2].value = inputs[0].value + "T" + inputs[1].value + "+03:00";
            return true;
        });

        var addBut = $('#add_tag');
        var addDel = $('#delete_tag');
        var tag_select = $('#tag_select');
        var component_tag_list = $('.component_tag_list')[0];
        var tag_list_hidden = $('.tag_list_hidden')[0];

        addBut.click(function() {
            var value = component_tag_list.value;
            if(value === '' && !!tag_select.val())
                value += tag_select.val();
            else {
                tagStatus = false;
                var mass_tag = value.split(', ');
                mass_tag.forEach(logArrayElements);
                if(!tagStatus && !!tag_select.val())
                    value += ', ' + tag_select.val();
            }
            component_tag_list.value = value;
            tag_list_hidden.value = value;
        });
        ...
        addDel.click(function() {
            ...
            if(value !== '') {
                ...
                mass_tag.forEach(logArrayElements);
                if(tagIndexDelete >= 0) {
                    mass_tag.splice(tagIndexDelete, 1);
                    value = mass_tag.join(', ');
                }
            }
            component_tag_list.value = value;
        ...
        function logArrayElements(element, index, array) {
            if(element === tag_select.val()) {
                tagIndexDelete = index;
                tagStatus = true;
            }
        }
        ...
        var tag_select = $('#tag_select');
        var warning_placeholder = function( event ) {
            if(!$("#acts_as_taggable_on_tag_name").val()){
                $("#acts_as_taggable_on_tag_name").attr("placeholder", "Заполните имя тега");
                event.preventDefault();
            }
        };

        $( ".new_acts_as_taggable_on_tag" ).submit(function( event ) {
            warning_placeholder(event);
        });

        $( ".edit_acts_as_taggable_on_tag" ).submit(function( event ) {
            warning_placeholder(event);
        });
        ...
        $('#edit_tag_btn').click(function() {
            if( !!tag_select.val() )
                window.location.replace(window.location.href + '/' + tag_select.val() + '/' + 'edit');
        });

        $('#delete_tag_btn').click(function() {
            var id = tag_select.val();
            if(!!tag_select.val() && confirm( "Вы подтверждаете удаление?")) {
                $.ajax({
                    type: 'DELETE',
                    url: window.location.href + '/' + id + '/',
                    dataType: "json",
                    success: function (data) {
                        $("#tag_select [value='" + id + "']").remove();
                        $("#tag_select :last").attr("selected", "selected");
                        $("#tag_select").select2().select2('val', tag_select.val());
                    }
                });
            }
        });
